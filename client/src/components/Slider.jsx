    ...
    <div className="w-[95%] m-auto mt-4 bg-white relative overflow-hidden">
      ...
      >
        {images.map((img, i) => (
          <img
            key={i}
            className="w-full h-[160px] lg:h-[400px] sm:h-[400px] flex-shrink-0 flex-grow-0"
            src={img}
            alt=""
          />
        ))}
      ...
      <div className="md:flex top-0 w-full h-full justify-between items-center absolute text-3xl text-white px-5 hidden">
        <button>
          <FaArrowAltCircleLeft onClick={() => handleLeft()} />
        </button>
        <button>
          <FaArrowAltCircleRight onClick={() => handleRight()} />
        </button>
      </div>
      <div className="absolute w-full bottom-2 flex justify-center items-center gap-2">
        {images.map((img, i) => (
          <button
            ...
            className={`md:w-3 md:h-3 h-2 w-2 rounded-full cursor-pointer ${
