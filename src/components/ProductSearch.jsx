...
const ProductSearch = () => {
  ...
  const [activeSort, setActiveSort] = useState("Best Match");
    ...
    const filters = {
      categorys: [...new Set(state.searchProducts[0].map((product) => product.category))],
      brands: [...new Set(state.searchProducts[0].map((product) => product.brand || "No Brand"))],
    };

  const handleSortClick = (sortOption) => {
    setActiveSort(sortOption);

    let sortedProducts = [...state.searchProducts[0]];
  ...
  useEffect(() => {
    setFilterProducts([...state.searchProducts]); // Ensure products are reset when state changes
  }, [state.searchProducts]);

  const handleFilterProducts = (value) => {
    setFilterValue(value)
    let sortedProducts = [...state.searchProducts[0]].filter(product => product.brand === value || product.category === value);
     setFilterProducts([sortedProducts])
  }
  const sortOptions = [
    "Best Match",
    ...
    <div className="flex flex-col lg:flex-row lg:mx-8 gap-4 mb-2 mt-3">
      <div className="product-filter w-full lg:w-1/4 md:flex flex-col gap-5 p-4 bg-white drop-shadow-sm hidden">
        ...
        <div className="flex flex-col">
          ...
          {state.searchProducts[0] &&
            filters.categorys.map((category) => (
              <span key={uuidv4()} className={`pt-2 ${category === filterValue? 'text-blue-500': ''}`} value={category} onClick={() => handleFilterProducts(category)}>
                {category}
              </span>
            ))}
        </div>
            ...
            {state.searchProducts[0] &&
              filters.brands.map((brand) => (
                <label key={uuidv4()} className="pt-2">
                  <input type="checkbox" value={brand} onClick={() => handleFilterProducts(brand)} />
                  <span className="pl-2">
        ...
        <div className="product-price flex flex-col">
          ...
          <label className="pt-2">
            <input type="checkbox" name="option" value="50" onClick={() => handleFilterProducts(event)}/>
            <span className="pl-2">under - 50</span>
          </label>
          <label>
            <input type="checkbox" name="option" value="100" onClick={() => handleFilterProducts(event)}/>
            <span className="pl-2">50 - 100</span>
          </label>
          <label>
            <input type="checkbox" name="option" value="200" onClick={() => handleFilterProducts(event)} />
            <span className="pl-2">100 - 200</span>
          </label>
          <label>
            <input type="checkbox" name="option" value="201" onClick={() => handleFilterProducts(event)} />
      ...
      <div className="flex-1 bg-white drop-shadow-sm lg:p-4">
        ...
        <p className="font-semibold text-lg pb-2 px-4 lg:px-0">
          Showing {state.searchProducts[0].length} results for "{title}"
        </p>
          ...
          <div className="w-full bg-white p-4 shadow-lg absolute z-10">
            ...
            <div className="flex flex-col mb-4">
              ...
              <label className="pt-2 flex items-center cursor-pointer">
                <input type="checkbox" name="option" value="50" />
                <span className="pl-2">under - 50</span>
              </label>
              ...
              <label className="flex items-center cursor-pointer">
                <input type="checkbox" name="option" value="200" />
                <span className="pl-2">100 - 200</span>
              </label>
              <label className="flex items-center cursor-pointer">
                <input type="checkbox" name="option" value="201" />
                <span className="pl-2">200 - over</span>
              </label>
            </div>
            {/* Color Filter */}
