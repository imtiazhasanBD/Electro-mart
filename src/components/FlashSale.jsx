...
const FlashSale = () => {
    ...
    useEffect(() => {
        if (state.products && state.products[0]) {
          const flashSaleProducts = [...state.products[0]].sort((a, b) => b.discountPercentage - a.discountPercentage);
        ...
        <section className=' border-r-2'>
            ...
            <div>
                <span className='flex gap-2'>
                    ...
                    <p className=" text-red-500 text-sm">{state.saleProducts[0][0].discountPercentage}%</p>
                </span>
                <p className="price text-xl font-bold text-blue-500">${(state.saleProducts[0][0].price - (state.saleProducts[0][0].price/100 * state.saleProducts[0][0].discountPercentage)).toFixed(2)}
                </p>
            </div>
        </section>
        )
            ...
            state.saleProducts[0].slice(1,4).map((product) => (
                <div key={uuidv4()} className="flash-sale-product flex gap-2 pb-4">
                    ...
                    <span className='flex gap-2'>
                      ...
                      <p className=" text-red-500 text-xs">{product.discountPercentage}%</p>
                   </span>
                      <p className="price text-sm font-bold text-blue-500">${(product.price - (product.price/100 * product.discountPercentage)).toFixed(2)}
                      </p>
                    </div>
                </div>
        )
