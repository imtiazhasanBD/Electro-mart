  ...
  return (
    ...
    <div className='lg:ml-[80px]  mx-auto w-11/12 lg:p-8 p-2'>
      ...
      {state.favoriteProducts.length > 0 ? (
              ...
              {state.favoriteProducts.map(product => (
                <tr key={product.id} className="border-b hover:bg-gray-50">
                  <td className='py-3 px-4 text-center'>
                    <button onClick={() => removeFromFavorites(product.id)} className='text-blue-500 active:text-red-500'>
        ...
        <p className="text-gray-600">No favorite products yet. Browse and add some!</p>
      )}
    </div>
  )
}
